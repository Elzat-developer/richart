				...
				<section className="py-8 bg-white">
					<div className="container mx-auto pl-10 pr-4">
						<div className="text-center mb-12">
							<h2 className="text-3xl font-bold text-gray-900 mb-4">Бытовые категории</h2>
						</div>
						<div className="relative">
							<div ref={scrollContainerRef} className="flex gap-8 overflow-x-auto scrollbar-hide scroll-smooth">
								{[...Array(10)].map((_, index) => (
									<div key={index} className="flex-shrink-0 shadow-lg h-72 w-60 animate-pulse"
										style={{ backgroundColor: 'rgb(245,243,241)' }}></div>
								))}
							</div>
						</div>
					</div>
					...
					<div className="container mx-auto pl-10 pr-4">
						<div className="text-center py-12">
							...
							<button
								...
								className="bg-industrial-accent text-white px-6 py-2 rounded-lg 
												hover:bg-industrial-accent/90 transition-colors"
							>
								Попробовать снова
							</button>
						</div>
					...
					<div className="relative">
						...
						{categories.length > 0 && (
							<button
								...
								backdrop-blur-sm hover:bg-white/90 text-black p-2 rounded-full shadow-lg transition-colors"
							...
							{categories.map((category) => {
								...
								return (
									<Link
										...
										className="flex-shrink-0 group relative shadow-lg overflow-hidden transition-all
										 duration-300 hover:shadow-2xl hover:scale-x-110 hover:z-10 flex flex-col"
										style={{ backgroundColor: 'rgb(245,243,241)', width: '250px', height: '288px' }}
									>
										{/* Фото категории */}
										<div className="flex-1 overflow-hidden p-5 flex items-center justify-center"
											...
											<img
												...
												onError={(e) => {
													(e.target as HTMLImageElement).src =
														`https://picsum.photos/300/200?error=${categoryAny.categoryId || category.categoryId}`;
												}}
											/>
										</div>
